...
function SpeakerAvatar({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: {
    ...
    boxShadow: borderOpacity > 0 ? `0 0 ${size * 0.06}px ${hexA(borderColor, borderOpacity * 0.6)}` : "none"
  }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: face }) });
}
function hexA(hex2, a) {
  const n2 = parseInt(hex2.slice(1), 16);
  return `rgba(${n2 >> 16 & 255},${n2 >> 8 & 255},${n2 & 255},${a})`;
}
function samplePath(position, w2, h, count) {
  const pts = [];
  if (position === "circular") {
    const cx = w2 / 2;
    const cy = h / 2;
    const r2 = Math.min(w2, h) * 0.34;
  ...
  if (position === "top" || position === "bottom") {
    const y2 = position === "top" ? h * 0.08 : h * 0.92;
    const ny = position === "top" ? 1 : -1;
    for (let i = 0; i < count; i++) {
      const x22 = w2 * (0.08 + i / (count - 1) * 0.84);
      pts.push({ x: x22, y: y2, nx: 0, ny });
    }
    return pts;
  }
  const x2 = position === "left" ? w2 * 0.08 : w2 * 0.92;
  const nx = position === "left" ? 1 : -1;
  for (let i = 0; i < count; i++) {
    const y2 = h * (0.08 + i / (count - 1) * 0.84);
    pts.push({ x: x2, y: y2, nx, ny: 0 });
  }
  return pts;
}
function fakeAmplitude(track, i, timeMs) {
  const t2 = timeMs / 1e3;
  const phase = i * 0.7 + track * 2.1;
  const a = Math.sin(t2 * 2.4 + phase) * 0.5 + Math.sin(t2 * 5.1 + phase * 1.3) * 0.3 + Math.sin(t2 * 0.9 + phase * 0.4) * 0.2;
  return Math.max(0.05, Math.min(1, (a + 1) / 2));
}
function fakeActiveTrack(trackCount, timeMs) {
  if (trackCount <= 1) return 0;
  const cycle = 2400;
  return Math.floor(timeMs / cycle) % trackCount;
}
const SAMPLE_COUNT = 48;
function tracksForBehavior(cfg) {
  switch (cfg.behavior) {
    case "single":
...
function offsetPoints(base, position, lane) {
  if (lane === 0) return base;
  const laneGap = position === "circular" ? 14 : 10;
  return base.map((p2) => ({
    ...p2,
    x: p2.x + p2.nx * lane * laneGap,
    y: p2.y + p2.ny * lane * laneGap
  }));
}
function extend(p2, len) {
  return { x: p2.x + p2.nx * len, y: p2.y + p2.ny * len };
}
function toPathD(points, closed, smooth) {
...
function WaveformRenderer({ config, width, height }) {
  const [timeMs, setTimeMs] = reactExports.useState(0);
  const rafRef = reactExports.useRef();
  reactExports.useEffect(() => {
    const start = performance.now();
    const loop = (now2) => {
      setTimeMs(now2 - start);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "svg",
    {
      ...
      children: tracks.map((track, ti2) => {
        const isColorShift = config.behavior === "single-colorshift";
        const color2 = isColorShift ? fakeActiveTrack(2, timeMs) === 0 ? config.colorA : config.colorB : track.color;
        const active = track.alwaysActive || ti2 === activeIdx;
        const opacity = active ? 1 : 0.22;
        const points = offsetPoints(base, config.position, track.laneOffset);
        const amps = points.map(
          (_, i) => active ? fakeAmplitude(ti2, i, timeMs) : 0.06
        );
        const barWidth = Math.max(1.5, Math.min(width, height) * 0.9 / SAMPLE_COUNT / 2);
        switch (config.style) {
          case "bars": {
            return /* @__PURE__ */ jsxRuntimeExports.jsx("g", { opacity, children: points.map((p2, i) => {
              const tip = extend(p2, amps[i] * maxLen);
              return /* @__PURE__ */ jsxRuntimeExports.jsx(
                "line",
                {
                  x1: p2.x,
          ...
          case "mirror": {
            return /* @__PURE__ */ jsxRuntimeExports.jsx("g", { opacity, children: points.map((p2, i) => {
              const out = extend(p2, amps[i] * maxLen);
              const inn = extend(p2, -amps[i] * maxLen * 0.7);
          ...
          default: {
            ...
            const outline = points.map((p2, i) => extend(p2, amps[i] * maxLen));
            return /* @__PURE__ */ jsxRuntimeExports.jsx(
              "path",
              {
...
const useSettingsStore = create()(
  persist(
    (set) => ({
      ...
      setDefault: (k2, v2) => set((s) => ({ defaults: { ...s.defaults, [k2]: v2 } }))
    }),
    { name: "byok-settings" }
    // saved to localStorage, survives restarts
  )
);
...
const SPEAKER_COLORS = ["#e8a24a", "#4ac2e8"];
const useProjectStore = create((set) => ({
  ...defaultProject,
  setRender: (p2) => set((s) => ({ render: { ...s.render, ...p2 } })),
  setWaveform: (p2) => set((s) => ({ waveform: { ...s.waveform, ...p2 } })),
  setBgRelevancy: (v2) => set({ bgRelevancy: v2 }),
  setFps: (fps) => set({ fps }),
  addSpeaker: () => set((s) => {
    const n2 = s.speakers.length;
    const next = {
      ...
      borderOpacity: 1,
      bgColor: "#1a1a1a",
      borderColor: SPEAKER_COLORS[n2 % SPEAKER_COLORS.length],
  ...
  removeSpeaker: (id2) => set((s) => ({ speakers: s.speakers.filter((sp) => sp.id !== id2) })),
  updateSpeaker: (id2, patch) => set((s) => ({
    speakers: s.speakers.map(
      (sp) => sp.id === id2 ? { ...sp, ...patch } : sp
    )
  })),
...
const useTemplatesStore = create()(
  persist(
    (set) => ({
      templates: {},
      saveTemplate: (name, snapshot) => set((s) => ({
        templates: {
          ...s.templates,
          [name]: { ...snapshot, savedAt: Date.now() }
        }
      })),
      deleteTemplate: (name) => set((s) => {
...
function BackendPanel() {
  ...
  const [encryptionAvailable, setEncryptionAvailable] = reactExports.useState(true);
  ...
  const save = async (id2) => {
    if (!draft[id2]) return;
    await window.byok.keys.set(id2, draft[id2]);
    setDraft((d) => ({ ...d, [id2]: "" }));
    refresh();
  };
  const remove = async (id2) => {
    await window.byok.keys.remove(id2);
    refresh();
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-6 space-y-4 overflow-y-auto h-full", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "label-lit font-display uppercase tracking-[0.18em] text-sm", children: "API Keys & Backend" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-zinc-500", children: "Keys are encrypted with your OS keychain and never leave this machine." }),
    !encryptionAvailable && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-amber-400 border border-amber-900/50 bg-amber-950/30 rounded-lg px-3 py-2", children: "Your OS keychain isn't available, so keys are being saved to a local file instead of encrypted storage. They still never leave this machine, but consider unlocking your OS keychain for stronger protection." }),
    PROVIDERS.map((p2) => {
      const isSaved = saved.includes(p2.id);
      return /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          className: `rounded-xl border p-4 ${p2.future ? "border-zinc-800 bg-zinc-900/40 opacity-60" : "border-zinc-700 bg-zinc-900/80"}`,
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-sm text-zinc-200", children: [
                p2.label,
                p2.future && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-2 rounded bg-amber-900/40 px-2 py-0.5 text-[10px] text-amber-400", children: "COMING SOON" })
              ] }),
              isSaved && !p2.future && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-emerald-400", children: "saved ✓" })
            ] }),
            !p2.future && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  type: "password",
                  ...
                  value: draft[p2.id] ?? "",
                  onChange: (e) => setDraft((d) => ({ ...d, [p2.id]: e.target.value })),
            ...
            p2.id === "azure" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "text",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-xl border border-zinc-700 bg-zinc-900/80 p-4 space-y-3", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "label-lit font-display uppercase tracking-[0.18em] text-xs", children: "Saved Templates" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-zinc-500", children: "Captures render settings, waveform config, and speaker setup — not API keys or backend defaults." }),
      ...
      Object.keys(templates).length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-zinc-600", children: "No templates saved yet." }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-2", children: Object.entries(templates).map(([name, tpl]) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          className: "flex items-center justify-between rounded-lg border border-zinc-700 bg-zinc-950/60 px-3 py-2",
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-zinc-200", children: name }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-[10px] text-zinc-600", children: new Date(tpl.savedAt).toLocaleString() })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
...
const FPS_OPTIONS = [10, 24, 30];
const WAVEFORM_STYLES = ["bars", "lines", "wave", "mirror", "dots"];
const WAVEFORM_POSITIONS = ["circular", "top", "bottom", "left", "right"];
const WAVEFORM_BEHAVIORS = [
...
function App() {
  ...
  const [canvasSize, setCanvasSize] = reactExports.useState({ w: 0, h: 0 });
  const fps = useProjectStore((s) => s.fps);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "h-full w-full flex flex-col bg-metal-900 text-neutral-200", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "panel-metal m-3 px-6 py-3 flex items-center justify-between", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "font-display uppercase tracking-[0.25em] text-lg label-lit", children: "BYOK-Vid-Creator" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "label-etched hidden sm:inline", children: "Deterministic Video Studio" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: view === "canvas" ? "⚙ Backend Settings" : "← Back to Canvas"
          }
        )
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-1 gap-3 px-3 pb-3 min-h-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "panel-metal w-72 p-5 flex flex-col gap-7 overflow-y-auto", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "label-etched mb-2", children: "Frame Rate" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2", children: FPS_OPTIONS.map((f2) => /* @__PURE__ */ jsxRuntimeExports.jsx(PlasticButton, { active: fps === f2, onClick: () => setFps(f2), children: f2 }, f2)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "label-etched mb-2", children: "Aspect Ratio" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              PlasticButton,
              {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "label-etched mb-2", children: "Waveform Style" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-2", children: WAVEFORM_STYLES.map((s) => /* @__PURE__ */ jsxRuntimeExports.jsx(PlasticButton, { active: waveform.style === s, onClick: () => setWaveform({ style: s }), children: s }, s)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "label-etched mb-2", children: "Waveform Position" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-2", children: WAVEFORM_POSITIONS.map((p2) => /* @__PURE__ */ jsxRuntimeExports.jsx(PlasticButton, { active: waveform.position === p2, onClick: () => setWaveform({ position: p2 }), children: p2 }, p2)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "label-etched mb-2", children: "Waveform Mode" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-2", children: WAVEFORM_BEHAVIORS.map((b) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            PlasticButton,
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "label-etched mb-2", children: "Waveform Colors" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-3", children: [
            ["colorA", "Speaker A"],
            ["colorB", "Speaker B"],
            ["colorMusic", "Music"]
          ].map(([key, label]) => /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "flex flex-col items-center gap-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "color",
                value: waveform[key],
                onChange: (e) => setWaveform({ [key]: e.target.value }),
                className: "h-7 w-7 rounded-md border border-zinc-700 bg-transparent p-0"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] text-zinc-500", children: label })
          ] }, key)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "label-etched mb-2 flex items-center justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Speakers" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: addSpeaker, className: "text-amber-400 hover:text-amber-300 text-xs", children: "+ Add" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-2", children: [
            speakers.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-zinc-600", children: "No speakers yet." }),
              ...
              "div",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "inline-block h-2.5 w-2.5 rounded-full", style: { background: sp.borderColor } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-zinc-200", children: sp.label })
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => removeSpeaker(sp.id), className: "text-zinc-500 hover:text-red-400 text-xs", children: "✕" })
                ]
              },
              sp.id
            ))
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "panel-metal flex-1 grid place-items-center p-6 min-h-0", children: view === "settings" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full h-full", children: /* @__PURE__ */ jsxRuntimeExports.jsx(BackendPanel, {}) }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ref: canvasRef,
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(WaveformRenderer, { config: waveform, width: canvasSize.w, height: canvasSize.h }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "label-etched text-center leading-relaxed relative z-10", children: [
              render.format,
              " · ",
              fps,
              " FPS",
            ...
            speakers.map((sp) => /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                style: {
                  position: "absolute",
